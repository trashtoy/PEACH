...
class Peach_DT_DateTest extends Peach_DT_AbstractTimeTest
{
    ...
    /**
     * {@link Peach_DT_Time::TYPE_DATE} を返すことを確認します.
     * @covers Peach_DT_Date::getType
     */
    public function testGetType()
    {
        $d = new Peach_DT_Date(2012, 5, 21);
        $this->assertSame(Peach_DT_Time::TYPE_DATE, $d->getType());
    }
    
    /**
     * 形式が "YYYY-MM-DD" になっていることを確認します.
     * 特にフィールドが1桁の場合に '0' が付加されていることを確認します.
     * @covers Peach_DT_Date::__toString
     */
    public function test__toString()
    {
        $d = array(
            new Peach_DT_Date(2011,  5,  1),
            new Peach_DT_Date(2000, 12, 31),
            new Peach_DT_Date(1863, 11, 19)
        );
        $f = array(
            "2011-05-01",
            "2000-12-31",
     ...
     * - 時・分のフィールドが 0 になっている
     * 
     * @covers Peach_DT_Date::toDatetime
     */
    public function testToDatetime()
    {
        $d1 = new Peach_DT_Date(2012, 5, 21);
        $this->assertEquals(new Peach_DT_Datetime(2012, 5, 21, 0, 0), $d1->toDatetime());
    }
     ...
     * 
     * @covers Peach_DT_Date::toTimestamp
     */
    public function testToTimestamp()
    {
        $d1 = new Peach_DT_Date(2012, 5, 21);
        $this->assertEquals(new Peach_DT_Timestamp(2012, 5, 21, 0, 0, 0), $d1->toTimestamp());
    }
    ...
    {
        $d1 = new Peach_DT_Date(2011, 7, 8);
        $d2 = new Peach_DT_Date(2009, 11, 12);
        $d3 = new Peach_DT_Date(2010, 2, 4);
        $d4 = new Peach_DT_Date(2012, 2, 3);
    ...
    {
        $d1 = new Peach_DT_Date(2012, 5, 21);
        $this->assertEquals(new Peach_DT_Date(2015, 5,  21),  $d1->add("year",   3));
        $this->assertEquals(new Peach_DT_Date(2009, 5,  21),  $d1->add("year",  -3));
        $this->assertEquals(new Peach_DT_Date(2012, 10, 21),  $d1->add("month",  5));
        ...
        $this->assertEquals(new Peach_DT_Date(2012, 5,  21),  $d1->add("asdf",  20));
    }
    ...
    public function testAfter()
    {
        ...
        // 比較が正常にできる
        $this->assertTrue($d1->after(new Peach_DT_Date(2011, 12, 31)));
        $this->assertFalse($d1->after(new Peach_DT_Date(2013, 3, 1)));
        
        // 同じオブジェクトの場合は FALSE を返す
        $this->assertFalse($d1->after(new Peach_DT_Date(2012, 5, 21)));
        
        // 異なる型との比較で, 共通のフィールドが全て等しい場合は, フィールドが多いほうが「後」
        $this->assertFalse($d1->after(new Peach_DT_Datetime(2012, 5, 21, 0, 0)));
        $this->assertFalse($d1->after(new Peach_DT_Timestamp(2012, 5, 21, 0, 0, 0)));
    }
    ...
    public function testBefore()
    {
        ...
        // 比較が正常にできる
        $this->assertFalse($d1->before(new Peach_DT_Date(2011, 12, 31)));
        $this->assertTrue($d1->before(new Peach_DT_Date(2013, 3, 1)));
        
        // 同じオブジェクトの場合は FALSE を返す
        $this->assertFalse($d1->before(new Peach_DT_Date(2012, 5, 21)));
        
        // 異なる型との比較で, 共通のフィールドが全て等しい場合は, フィールドが少ないほうが「前」
        $this->assertTrue($d1->before(new Peach_DT_Datetime(2012, 5, 21, 0, 0)));
        $this->assertTrue($d1->before(new Peach_DT_Timestamp(2012, 5, 21, 0, 0, 0)));
    }
    ...
    {
        $d1 = new Peach_DT_Date(2012, 5, 21);
        $d2 = new Peach_DT_Date(2012, 1, 21);
    ...
    public function testFormat()
    {
        ...
        $this->assertSame("2012-05-21", $d->format(Peach_DT_W3cDatetimeFormat::getInstance()));
    }
    ...
    {
        $time        = new Peach_DT_Date(2012, 5, 21);
        $valid       = array();
    ...
    public function testSet()
    {
        ...
        $this->assertEquals(
            array(
                new Peach_DT_Date(2013, 5, 21),
                new Peach_DT_Date(0,    5, 21),
                new Peach_DT_Date(999,  5, 21),
                new Peach_DT_Date(9999, 5, 21)
            ),
            array(
                $time->set("y", 2013),
                $time->set("y", 10000),
        ...
        $this->assertEquals(
            array(
                new Peach_DT_Date(2012, 10, 21),
                new Peach_DT_Date(2013,  1, 21),
                new Peach_DT_Date(2014,  2, 21),
                new Peach_DT_Date(2011, 12, 21),
                new Peach_DT_Date(2010, 11, 21)
            ), 
            array(
                $time->set("mo", 10),
                $time->set("mo", 13),
        ...
        $this->assertEquals(
            array(
                new Peach_DT_Date(2012, 6, 1),
                new Peach_DT_Date(2012, 4, 30),
                new Peach_DT_Date(2012, 4, 1),
                new Peach_DT_Date(2012, 7, 3),
                new Peach_DT_Date(2012, 3, 2)
            ), 
            array(
                $time->set("d", 32),
                $time->set("d", 0),
     ...
     * 
     * @covers Peach_DT_Date::setAll
     */
    public function testSetAll()
    {
        $d     = new Peach_DT_Date(2012, 5, 21);
        $test1 = $d->setAll(array("year" => 2015, "date" => 10));
        $this->assertEquals(new Peach_DT_Date(2015, 5, 10), $test1);
        
        $map  = new Peach_Util_ArrayMap();
        $map->put("month", 12);
        $map->put("date",  31);
        $test2 = $d->setAll($map);
        $this->assertEquals(new Peach_DT_Date(2012, 12, 31), $test2);
        
        // 2011-14-31 => 2012-02-31 => 2012-03-02
        $this->assertEquals(new Peach_DT_Date(2012, 3, 2), $d->setAll(array("year" => 2011, "month" => 14, "date" => 31)));
    }

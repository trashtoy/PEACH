...
class Peach_DT_UtilTest extends PHPUnit_Framework_TestCase
{
     ...
     * 
     * @covers Peach_DT_Util::compareTime
     */
    ...
    {
        $d = $this->getTestArray();
        $this->assertSame($d[0], Peach_DT_Util::oldest($d));
        $this->assertSame($d[1], Peach_DT_Util::oldest($d[4], $d[1], $d[3], $d[5], $d[8]));
        $this->assertSame($d[2], Peach_DT_Util::oldest($d[5], null, $d[2], 128, $d[3]));
    }
    ...
    {
        $d = $this->getTestArray();
        $this->assertSame($d[8], Peach_DT_Util::latest($d));
        $this->assertSame($d[6], Peach_DT_Util::latest($d[4], $d[2], $d[6], $d[0], $d[5]));
        $this->assertSame($d[7], Peach_DT_Util::latest($d[1], 256, $d[7], false, $d[3]));
    }
     ...
     * の場合のみ OK とします.
     * 
     * @covers Peach_DT_Util::validate
     */
    public function testValidate()
    {
        $this->assertTrue(Peach_DT_Util::validate(2012, 2, 29));
        $this->assertTrue(Peach_DT_Util::validate(2012, 5, 21, 18, 30));
        $this->assertTrue(Peach_DT_Util::validate(2012, 3,  1, 23,  0, 30));
        $this->assertTrue(Peach_DT_Util::validate(2012, 5, 21,  7, 30, "1"));
        $this->assertFalse(Peach_DT_Util::validate(2011, 2, 29));
        $this->assertFalse(Peach_DT_Util::validate(2012, -1, 1));
        $this->assertFalse(Peach_DT_Util::validate(2012, 5, 21, 25, 0, 30));
        $this->assertFalse(Peach_DT_Util::validate(2012, 5, 21, 1, 0, -1));
        $this->assertFalse(Peach_DT_Util::validate(2012, 5, 21, 7, 30, "hoge"));
    }
    
    /**
     * システムの時差を分単位で取得することを確認します.
     * @covers Peach_DT_Util::getTimeZoneOffset
     */
    public function testGetTimeZoneOffset()
    {
        $this->assertSame(-540, Peach_DT_Util::getTimeZoneOffset());
    }
     ...
     * - 数値以外の値は整数に変換する
     * 
     * @covers Peach_DT_Util::cleanTimeZoneOffset
     */
    public function testCleanTimeZoneOffset()
    {
        ...
        for ($i = 0; $i < 5; $i ++) {
            $this->assertSame($expected[$i], Peach_DT_Util::cleanTimeZoneOffset($test[$i]));
        }
    }
}
